...
class Supervisor_model extends CI_Model{
    ...
    public function contasAPagarSemana(){
        ...
        $result = $this->db->query("SELECT SUBSTRING(dayname(start), 1, 3) as dia, dayofweek(start), SUM(valor) as totalSemana FROM tbl_gasto WHERE WEEKOFYEAR(start) = WEEKOFYEAR(now()) AND id_status = 2 GROUP BY dayname(start) ORDER BY dayofweek(start)");
